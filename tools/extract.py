...
1. `src/renderer/src/i18n/locales/en.json` из репозитория Orca — берётся
   ...
   целиком. Полный каталог релиза — 14 099 ключей для 1.4.198.

2. Разбор `app.asar` — страховка: добавляет ключи, которых в `en.json` нет.
   До 1.4.197 так добирались строки поиска по настройкам (~152 ключа):
   они объявлялись через `translateSearchKeyword`, которого не знал
   экстрактор. С 1.4.198 (PR stablyai/orca#18040) они в каталоге, и добор
   равен нулю — ненулевое число здесь снова повод разобраться.
